...
class AllRecipeRecyclerAdapter(val onClickListener: RecipeOnClickListener,val databaseDao: RecipeDao, val profileDao: ProfileDao, val profileId : Long, val favouriteDao: FavouriteDao) : ListAdapter<DataItemRecipe, AllRecipeRecyclerAdapter.ViewHolder>(RecipeDiffCallback())
{
    ...
    private fun getFavourites(profileId : Long, favoriteDao : FavouriteDao) = favoriteDao.getAllProfileFavouirtes(profileId)
    ...
    fun submitNewRecommendedList(){
        adapterScope.launch {
            ...
            for(recipe in recipes!!){
                if(!checkCaffeine(recipe, profile!!) && !checkFructose(recipe, profile!!) && !checkGluten(recipe, profile!!) && !checkLactose(recipe, profile!!)){
    ...
    class ViewHolder private constructor(val binding : RecipeListItemBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(item: Recipe, onClickListener: RecipeOnClickListener){
            ...
            Glide.with(binding.recipeImage).load(item.photoUrl).optionalCenterCrop().into(binding.recipeImage)
